...
if [ ! $# -gt 0 ]; then
    RELEASES=$(
        curl -s \
            ...
            https://api.github.com/repos/vestainnovations/karpenter-provider-aws/releases
    )
...
RESPONSE=$(
    gh api \
        -H "Accept: application/vnd.github+json" \
        /repos/vestainnovations/karpenter-provider-aws/compare/$PREVIOUS...$LATEST?per_page=$COMMITS_PER_PAGE
)
...
for i in $(seq 1 $PAGES); do
    NEXT=$(
        gh api \
            -H "Accept: application/vnd.github+json" \
            /repos/vestainnovations/karpenter-provider-aws/compare/$PREVIOUS...$LATEST?per_page=$COMMITS_PER_PAGE\&page=$i | jq -r ".commits"
    )
